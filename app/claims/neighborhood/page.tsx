...
import { SearchClaimsByNeighborhood } from "@/app/components/claim/SearchClaimsByNeighborhood";

const fetchClaims = async (params: ParamType | null) => {
  try {
    if (params) {
      const result = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/claims?neighborhood=${params.neighborhood}`,
        {
          next: { tags: ["claims"] },
...
export default async function Claims() {
  ...
  return (
    <ProtectedAdmin>
      <>
        <h1>Заявки по Адм.район</h1>
        {claims.data && (
          <>
            <SearchClaimsByNeighborhood />
            <ClaimsTable
